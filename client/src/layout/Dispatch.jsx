...
export const Dispatch = () => {
  ...
  const onDispatchHandler = (oldOrder) => {
    ...
    axios.put(`http://localhost:8000/api/order/status/${oldOrder}/dispatch`)
      .then(res => { setAllOrders(allOrders.filter(order => order._id !== oldOrder)) })
        ...
        <div className='row'>
          {allOrders.length > 0 && allOrders.map((item, index) => {
            if (item.status === 'en espera') {
              return (<div key={index} className='col-lg-3 mt-5'>
                <div className="col-sm card text-dark bg-light mb-3" style={{ width: '18rem' }}>
                  {/* <img src="..." class="card-img-top" alt="..." /> */}
                  <h1 className='card-header bg-secondary text-center'>{item.desk}</h1>
                  <div className="card-body">
                    ...
                    <div className='text-center'>
                      ...
                      {/* <button onClick={() => { handlerEdit(item) }} type="button" className="btn btn-warning me-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
                      Editar Orden
                    </button> */}
